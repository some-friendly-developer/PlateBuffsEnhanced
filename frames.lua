...
local function CreateBuffFrame(parentFrame, realPlate)
	...
	f.realPlate = realPlate f:SetFrameStrata("BACKGROUND")
	f.icon = CreateFrame("Frame", nil, f) f.icon:SetPoint("TOP", f)
	f.texture = f.icon:CreateTexture(nil,"BACKGROUND")
	f.texture:SetAllPoints(true)

	local cd = f:CreateFontString(nil, "ARTWORK", "ChatFontNormal") cd:SetText("0")

	cd:SetPoint("TOP", f.icon, "BOTTOM")
	f.cd = cd --Make the text easier to see.
	f.cdbg = f:CreateTexture(nil,"BACKGROUND")
	f.cdbg:SetTexture(0,0,0,.75)
	f.cdbg:SetPoint("CENTER", cd) --clock cooldown texture
	...
	f.cdtexture:SetReverse(true) f.cdtexture:SetDrawEdge(false)
	...
	f.stack:SetText("0") f.stack:SetPoint("BOTTOMRIGHT", f.icon, "BOTTOMRIGHT", -1, 3)
	...
	f.lastUpdate = 0 f.expirationTime = 0
	...
	f.cdtexture:Hide()
	f.stack:Hide()
	--[===[@debug@ --~ f:EnableMouse(true) --~ f:SetScript("OnEnter", function(frame) --~ Debug("OnEnter!", --~ frame.spellName, --~ frame.expirationTime, --~ (frame.expirationTime or 0) - GetTime(), --~ frame.cd:IsShown() --~ ) --~ end) --@end-debug@]===]
	--[[ if LBF then local Group = LBF:Group(folder) local skinID = Group.SkinID local SkinData = LBF:GetSkins() Debug("Create", skinID, SkinData[skinID].Border.Texture) f.skin = f.icon:CreateTexture(nil,"OVERLAY") f.skin:SetAllPoints(true) f.skin:SetTexture(SkinData[skinID].Border.Texture) f.skin:SetTexCoord(0.2,0.8,0.2,0.8) f.skin:SetBlendMode("ADD") f.skin:Hide() end]]
	return f
end
